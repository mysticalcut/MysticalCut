 ...
 property: */
  ...
  public function findId(int $id): array
  {
    try {
      $this->conn = new ConnectDB();
      $this->pdo = $this->conn->connect();
      $this->sql = "SELECT * FROM facture WHERE $this->primaryKey ={$id}";
      $result = $this->pdo->prepare($this->sql);
      $result->execute();
      $results = $result->fetchAll(PDO::FETCH_ASSOC);
      $this->data = $results;
   ...
   */
  public function create(array $facture): array
  {
    try {
      if ($this->validateModel($facture)) {
        $this->conn = new ConnectDB();
        $this->pdo = $this->conn->connect();
        $this->sql = "INSERT INTO facture(fecha, valor_total,user_fk) VALUES (?,?,?)";
        $stmt = $this->pdo->prepare($this->sql);

        $stmt->bindParam(1, $facture[$this->modelData[0]]);
        $stmt->bindParam(2, $facture[$this->modelData[1]]);
        $stmt->bindParam(3, $facture[$this->modelData[2]]);
        $stmt->execute();
   ...
   */
  public function update(array $facture, int $id): array
  {
    try {
      if ($this->validateModel($facture)) {
        $this->conn = new ConnectDB();
        $this->pdo = $this->conn->connect();
        $this->sql = "UPDATE facture SET valor_total=?,user_fk=? WHERE  $this->primaryKey=?";
        $stmt = $this->pdo->prepare($this->sql);
        $stmt->bindParam(1, $facture[$this->modelData[1]]);
        $stmt->bindParam(2, $facture[$this->modelData[2]]);
        $stmt->bindParam(3, $id);
        $stmt->execute();
  ...
  public function delete(int $id): array
  {
    try {
      $this->conn = new ConnectDB();
      $this->pdo = $this->conn->connect();
      $this->sql = "DELETE FROM facture WHERE  $this->primaryKey=?";
      $stmt = $this->pdo->prepare($this->sql);
      $stmt->bindParam(1, $id);
      $stmt->execute();
